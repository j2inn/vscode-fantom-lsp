...
public class FantomDebugSession {
    ...
    public void launch(JsonObject args) throws Exception {
        ...
        //
        // The real config.props of some FIN installations already sets
        //   java.options=-agentlib:jdwp=...,suspend=n,address=5005
        // which means any running FIN instance holds 5005.  We strip that
        // line in the shadow, but if shadow creation fails the raw config
        // would be used — catching the failure and aborting early avoids
        // a confusing "Address already in use" error in that case.
        ...
        consoleLog("[Fantom Debug] JDWP port: " + (port < 0 ? "disabled" : port));
        ...
        if (shadowHome == null && !noDebug) {
            // Shadow creation failed. If the real config.props already has a
            // JDWP entry on a different port (common in FIN installs) we would
            // either collide or fail to attach.  Abort with a clear message.
            ...
            throw new RuntimeException(
                ...
                ". Check stderr for details. If the installation's config.props " +
                "already has java.options with JDWP configured you may see " +
                "\"Address already in use\" without this protection.");
        }
        if (shadowHome != null) {
            consoleLog("[Fantom Debug] Shadow FAN_HOME: " + shadowHome);
        }
        ...
        consoleLog("[Fantom Debug] Launching: " + cmdDisplay);
        ...
        attachWithRetry("localhost", port, num(args, "startupTimeout", 60));

        // After attaching we must strip java.options from the shadow config.props.
        // fin (and its fanlaunch helper) starts child JVM processes for docs
        // generation, projkit, etc.  Every child inherits our FAN_HOME and reads
        // config.props — if java.options still contains the JDWP address they all
        // try to bind the same port and fail with "Address already in use".
        // Rewriting config.props here (AFTER the parent JVM has already started
        // its JDWP agent) removes the JDWP line so only the parent is debuggable.
        if (shadowHome != null) {
            stripJdwpFromShadowConfig(shadowHome);
        }
    }

    /** Rewrite the shadow etc/sys/config.props to remove java.options. */
    private void stripJdwpFromShadowConfig(String shadowHome) {
        try {
            ...
            String content = new String(Files.readAllBytes(cfg), java.nio.charset.StandardCharsets.UTF_8);
            ...
            consoleLog("[Fantom Debug] Stripped JDWP from shadow config.props (child processes won't try to bind the debug port).");
        } catch (Exception e) {
            System.err.println("[JDI] Warning: could not strip JDWP from shadow config.props: " + e);
        }
    }

    /** Send a line to the VS Code Debug Console (category "console"). */
     ...
     */
    /**
     * Poll for the JDWP agent to become ready, retrying every 500ms.
     *
     * Large Fantom apps (intelliplant, full FIN stacks) can take 20-40 seconds
     * to start their JVM.  A fixed 1.5s sleep followed by a single 10s attempt
     * is not nearly enough.  We poll instead: each attempt uses a 1.5s socket
     * timeout so we fail fast when the process isn't ready yet, and we keep
     * retrying until either we connect or the startup budget is exhausted.
     *
    ...
    private void attachWithRetry(String host, int port, int timeoutSecs) throws Exception {
        ...
        consoleLog("[Fantom Debug] Waiting for JVM on port " + port + " (up to " + timeoutSecs + "s)...");

        while (System.currentTimeMillis() < deadline) {
            // Check if the process died before JDWP even started
            if (fanProcess != null && !fanProcess.isAlive()) {
                throw new RuntimeException(
                    "Fantom process exited before the JDWP agent became ready (exit code " +
                    fanProcess.exitValue() + "). Check the Debug Console for process output.");
            }

            attempt++;
            try {
                attachToVm(host, port, 1500);
                consoleLog("[Fantom Debug] Connected to JVM after " + attempt + " attempt(s).");
                return;  // success
            } catch (Exception e) {
                lastError = e;
                // Print a progress dot every 5 attempts (~2.5s)
                if (attempt % 5 == 0) {
                    consoleLog("[Fantom Debug] Still waiting... (" + attempt + " attempts, " +
                        ((System.currentTimeMillis() - (deadline - timeoutSecs * 1000L)) / 1000) + "s elapsed)");
                }
                Thread.sleep(500);
            }
        }
        ...
        throw new RuntimeException(
            ...
            ". If your application needs more startup time, add \"startupTimeout\": 120 to your launch.json.");
    }
    ...
    private void attachToVm(String host, int port, int timeoutMs) throws Exception {
        ...
        // Filter to fan.* classes only — Fantom breakpoints can only be set on
        // fan.* JVM classes.  Without this filter the JVM sends a ClassPrepareEvent
        // for every single class loaded (java.*, jdk.*, sun.*, thousands of
        // framework classes during FIN/intelliplant startup).  Each event suspends
        // the loading thread even with SUSPEND_EVENT_THREAD; the event queue floods,
        // the event thread saturates, and "Generating docs" effectively freezes.
        // Also use SUSPEND_NONE: we don't need the class-loader thread suspended
        // just to install a breakpoint — installPendingBreakpoints() only needs the
        // ReferenceType object which is available regardless of thread state.
    ...
    private String createDebugShadowHome(String fanExe, int jdwpPort) {
        ...
        try {
            ...
            // Track whether we wrote config.props — we must write it even if
            // it doesn't exist in the real installation (some FIN builds omit
            // it and use defaults, but we need it to inject JDWP).
                            ...
                            try (DirectoryStream<Path> sysStream = Files.newDirectoryStream(entry)) {
                                for (Path sysFile : sysStream) {
                                    ...
                                    if ("config.props".equals(sysName)) {
                                        writeConfigProps(sysDest, Files.readAllBytes(sysFile), jdwpPort);
            ...
            // Ensure etc/sys/config.props always exists in the shadow even if
            // the real installation doesn't have one (FIN installs without an
            // explicit config.props still need our JDWP and debug=true entries).
            if (!wroteConfigProps) {
    ...
    /** Write a shadow etc/sys/config.props: strip existing java.options, ensure debug=true, inject JDWP. */
    private static void writeConfigProps(Path dest, byte[] originalBytes, int jdwpPort) throws IOException {
        String original = new String(originalBytes, java.nio.charset.StandardCharsets.UTF_8);
        // Strip java.options (avoids double-JDWP) — handles both LF and CRLF
     ...
     */
    /**
     * Reserve a port by keeping a ServerSocket bound until the caller
     * closes it.  Tries {@code preferred} first; if already taken, falls
     * back to an OS-assigned ephemeral port (bind on 0).
     *
     * Keeping the socket open eliminates the TOCTOU race between "is this
     * port free?" and the JVM JDWP agent binding it: nothing else can grab
     * the port while our socket is alive.  The caller MUST close the returned
     * socket right before starting the target process.
     */
                ...
                try {
                    ...
                    for (Event event : eventSet) {
                        if (!(event instanceof ClassPrepareEvent)) {
                            // ClassPrepareEvent volume is too high to log (thousands during startup)
